...
class Bock:
    #достаем бочонки по 1 штуке рандомом
    def f(self):
...
def main():
    game = Loto(2)
    bock = Bock(90)
    pl1 = Player('Ваша карточка')
    pl2 = Player('Карточка компьютера')

    while True:
        numbock = next(bock.gen)
        pl1.get_card(game.carduser)
        pl2.get_card(game.cardpk)

        vvod = input('У вас есть такая цифра? (+/-)')
        if vvod == '+':
            if pl1.search(game.carduser, numbock):
                continue
            else:
                print('Вы проиграли:(')
                sys.exit(1)
        if vvod == '-':
            if pl1.search(game.carduser, numbock):
                print('Вы проиграли:(')
                sys.exit(1)
            elif pl2.search(game.cardpk, numbock):
                continue
        if vvod != '+' and vvod != '-':
            print('Введите + или -')
            time.sleep(1)
